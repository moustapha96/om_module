...
class OrangeMoneyConfig(models.Model):
    ...
    def create_payment_order(self, amount, currency, order_id, transaction_id, customer_msisdn, description, reference, success_url , cancel_url=None):
        """
    ...
    # def fetch_api_transactions(self):
    #     """Récupérer les transactions depuis l'API Orange Money"""
    #     try:
    #         token = self._get_access_token()
    #         headers = {
    #             'Authorization': f'Bearer {token}',
    ...
    #         response = requests.get(
    #             f"{self.base_url}/api/eWallet/v1/transactions",
    #             headers=headers,
    #             timeout=30
    #         )
    ...
    #                 # Rechercher une transaction existante avec le même transactionId
