...
class SaleOrder(models.Model):
    ...
    def action_initiate_orange_money_payment(self):
        ...
        try:
            ...
            _logger.error(f"Erreur lors de l'initiation du paiement Orange Money pour la commande {self.name}: {str(e)}")
    ...
    def _validate_orange_money_payment(self):
        ...
        if self.state not in ['draft', 'sent', 'sale']:
            return {'success': False, 'message': 'La commande doit être en état brouillon, envoyée ou confirmée.'}

        # Vérifier le montant
        if self.amount_total <= 0:
            return {'success': False, 'message': 'Le montant de la commande doit être supérieur à zéro.'}
    ...
    def _process_api_success(self, response, payment_data, config):
        ...
        transaction_data = {
            'success_url': payment_data.get('success_url', 'https://www.ccbmshop.sn/'),
            'cancel_url': payment_data.get('success_url', 'https://www.ccbmshop.sn/'),
    ...
    def action_view_orange_money_transactions(self):
        """Action pour voir les transactions Orange Money de cette commande"""
        self.ensure_one()
